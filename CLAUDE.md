...
<!-- CLAUDE_REVIEW:CLAUDE_ASSUMPTIONS:START -->
<!-- CLAUDE_REVIEW:CLAUDE_ASSUMPTIONS:END -->
...
4. When a custom prompt proves reusable, move it into the bank and remove it from the active file. Use `python tasks.py reset-jobs` when changing postings so stale extras do not carry forward.
5. Generate the smallest useful packet with `python tasks.py claude-packet --mode ...`.
6. Generate the matching review prompt with `python tasks.py claude-prompt review --packet-mode ...`.
7. After Claude returns findings, generate the plan prompt with `python tasks.py claude-prompt plan --packet-mode ...`.
8. Give Claude's implementation plan or review notes to Codex for edits and validation.
9. Let Codex run scripts, validate generated Word documents, and keep final outputs in `output/`.
